...
export class DashboardComponent implements OnDestroy {
  ...
  datosTabla = [
    // Arreglo de datos para una tabla
    {
      columna1: 'Unidad de negocio de prueba ',
      columna2: '$0.00',
      columna3: '$0.00',
      columna4: '$0.00',
      columna5: '$0.00',
  ...
  constructor(
    ...
    private infoLog: InfoLoginService,
    private Infob: InfoBancosService,
    ...
    this.infoCuentaClabeService.buscarPbluConCuenta(res).subscribe((data) => {
      ...
      this.infoCuentaClabeService.buscarCuentaExiste(clabe).subscribe((d) => {
        if (d == null) {
          ...
          if (this.localStorageService.getData('cuenta')) {
            if (
  ...
  ngOnDestroy(): void {
    //Cuando sale del este pagina destruye todo proceso que este haciendo internamente en este caso el resultado de tiempo real deja de consultarse
  ...
  empezar() {
    //Este metodo depende de que si el check esta habilitado o npo ya que si esta hablilitado empiza hacer la consulta y si no deja de hacer la consulta
    /*if (this.isChecked) {
      this.stopDataUpdate();
  ...
  startDataUpdate(): void {
    // Iniciamos el intervalo de 15 segundos y nos suscribimos a él
    const datePipe = new DatePipe('en-US');
    let ini = datePipe.transform(this.fechaActual, 'yyyy-MM-dd');
    let dat = {
      idPblu: this.pblu,
  ...
  stopDataUpdate(): void {
    //para el proceso de consulta para realizar otra consulta filtrada por fecha
    if (this.intervalSubscription) {
      this.intervalSubscription.unsubscribe();
  ...
  elegir(da: any) {
    //Realiza el primer filtrado dependiendo de la primer fecha elegida
    const datePipe = new DatePipe('en-US');
  ...
  elegir2(da: any) {
    //Realiza el segundo filtrado dependiendo de la segunda fecha elegida
    const datePipe = new DatePipe('en-US');
