...
export class EnviarPagoComponent implements OnInit {
  ...
  constructor(
    ...
    private infoPagos: InfoPagosService,
    private enlistarSpei: InfoPagosService,
  ...
  ngOnInit(): void {
    ...
    if (this.localStorageService.getData('segundoPerfil')) {
      this.segundoPerfil = true;
      //this.cuentasControl.value = this.localStorageService.getData('cuenta');
      this.numeroDeCuenta = this.localStorageService.getData('cuenta');
    }
  }
  parametroCuentaConcentradora() {
    let pblu = { peiyu: this.localStorageService.getUsuario('pblu') };
    this.infoCuentaClabeService
      .parametrosCuentaConcentradora(pblu)
      .subscribe((dato) => {
        this.paramConfig = dato;
      });
  }
  listarCuentasClabes() {
    //Lista las cuentas clabes que tenemos en configuraciíon de cuenta
    let res = { peiyu: this.localStorageService.getUsuario('pblu') };
    this.infoCuentaClabeService
      .listarPagosDeAutorizarPblu(res)
      .pipe(
        catchError((error) => {
          this.openSnackBar(
  ...
  validarSeleccionManual() {
    this.seleccionManual =
      this.cuentasControl.value !== null && this.cuentasControl.value !== '';
  }
  ...
  private _filterBancos(value: string): any[] {
    ...
    return this.listaBancos.filter((banco) =>
      banco.descripcion.toLowerCase().includes(filterValue)
    );
  }
  private _filteCuenta(value: string): any[] {
    const str = String(value); // Convertir la variable event en una cadena de texto
    const filterValue = str.toLowerCase();
    return this.listaCuentas.filter((cuenta) =>
      cuenta.clabe.toLowerCase().includes(filterValue)
    );
  }
  ...
  enviar() {
    ...
    if (this.numeroDeCuenta.startsWith('659')) {
      if (this.clabeMadre === this.numeroDeCuenta.toString().trim()) {
        if (this.paramConfig.valor == 1) {
          ...
          speiout.bancoDestino =
            this.institucionControl.value.id_banco.toString();
          ...
          this.infoPagos
            .realizarPago(speiout)
            .pipe(
              catchError((error) => {
                this.openSnackBar(
                  'Error al generar la operación, Intente nuevamente',
                  'Aviso'
                );
          ...
          this.openSnackBar(
            'No se puede realizar el spei desde una cuenta concentradora',
            'Aviso'
          );
        }
      } else {
      ...
      this.openSnackBar('La clabe ordenante es incorrecta.', 'Aviso');
    }
  }
    ...
    if (this.numeroDeCuenta.startsWith('659')) {
      if (this.clabeMadre === this.numeroDeCuenta.toString().trim()) {
        if (this.paramConfig.valor == 1) {
          ...
          this.enlistarSpei
            .guardarEnLatablaListarPagos(dataBaseSPEI)
            .subscribe((data) => {
